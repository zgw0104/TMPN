...
from hetsgg.layers import Label_Smoothing_Regression
from hetsgg.modeling.matcher import Matcher
...
class RelationLossComputation(object):
    ...
    def __init__(
        ...
        self.use_label_smoothing = True #use_label_smoothing
    ...
    def __call__(self, proposals, rel_labels, relation_logits, refine_logits, add_data=None): # refine : entity logit
        """
        Computes the loss for relation triplet.
        ...
        if len(torch.nonzero(rel_labels != -1)) == 0:
            ...
            loss_relation = self.criterion_loss(relation_logits[rel_labels != -1], # Cross Entropy Loss
