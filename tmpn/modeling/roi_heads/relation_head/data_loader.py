...
class DataLoader:
    def __init__(self, data_dir, dataset):
        # self.args = args
        self.name = data_dir.split('/')[-2]  #'/data/wjx/kgicl/processed_data/fb237_v1/train'

        self.kg = KG(data_dir)
        self.build_graph(self.kg.background)
        self.name = dataset #vg
    ...
    def get_neighbors(self, nodes, subs, rels, objs=None, mask_relations=None, training=False):
        ...
        node_1hot = torch.sparse_coo_tensor(indices_, torch.ones(len(nodes)).cuda(), #[num_ent, batch_size]
                                            torch.Size([self.kg.entity_num, nodes.shape[0]]))   #表示每个实体的batch的关系

        edge_1hot = torch.sparse.mm(self.M_sub, node_1hot)  # edge_idx x batch_idx    M_sub:每个头实体和fact的关系
        edges = edge_1hot.indices()  #筛选和节点相关的边


        selected_edges = torch.index_select(self.graph, 0, edges[0])  #筛选边后的三元组  
        sampled_edges = torch.cat([edges[1].unsqueeze(1), selected_edges], dim=1).long()  #和batch索引拼接的三元组   [batch_idx,head_idx,rel_idx,tail_idx] [num_filter_edge,4]   与节点相关的所有边的索引
        if training:  #训练时过滤掉与query相关的边
        ...
        if training:# and not finetune:
            for i in range(len(subs)):
                sample_mask = (sampled_edges[:, 0] == i)#选择当前批次的边
                if objs is not None:
                    subs_mask = (sampled_edges[:, 1] == subs[i]) #选择与查询的头实体相关的边 
                    rels_mask = (sampled_edges[:, 2] == rels[i])    #选择与查询的关系相关的边
                    objs_mask = (sampled_edges[:, 3] == objs[i]) #选择与查询的尾实体相关的边

                    # Combine all the masks to filter out the unwanted edges
                    final_mask = sample_mask & subs_mask & rels_mask & objs_mask #组合所有条件，过滤掉与查询相关的边

                    subs_mask = (sampled_edges[:, 3] == subs[i])#选择与查询的头实体相关的反向边
                    rel_inv = self.kg.relation_num//2 + rels[i] if rels[i] < self.kg.relation_num//2 else rels[i] - self.kg.relation_num//2 #
                    rels_mask = (sampled_edges[:, 2] == rel_inv) #选择与查询的反向关系相关的边
                    objs_mask = (sampled_edges[:, 1] == objs[i]) #选择与查询的尾实体相关的反向边

                    # Combine all the masks to filter out the unwanted edges
                    final_mask = final_mask | (sample_mask & subs_mask & rels_mask & objs_mask) #组合所有条件，过滤掉与查询相关的边
                else:
        ...
        sampled_edges = torch.cat([sampled_edges, self_loop_edges], 0)  #为每个节点添加自环边，并添加到edgeset中
    ...
    def build_graph(self, triples):
        ...
        indices_A = torch.cat([torch.arange(self.fact_num).long().unsqueeze(1).cuda(), self.graph[:, 0].unsqueeze(1)], dim=1).t().cuda()  #每个三元组的头实体索引
...
class KG:
    def __init__(self, path):
        ...
        self.train_batch_size = 75 #75
        self.test_batch_size = 512 #512
        ...
        for rel in tqdm(os.listdir(os.path.join(path, 'cases'))):
            ...
            num_cases = min(len(case_files)//3, int(5))  #shot=5
    ...
    def get_case_graph(self, rels, random_order):
        ...
        for i, rel in enumerate(rels):
            ...
            batch_edge_query_relations.append(
                torch.ones(edges_type.shape).cuda() * query_relation + i * (self.relation_num)) #加上偏移量
        ...
        if len(batch_edge_index) > 0 :
            ...
            is_None = False
            return batch_edge_index, batch_edge_type, batch_h_positions, batch_t_positions, batch_query_relations, batch_edge_query_relations, batch_labels, num_ent,is_None
        else:
            is_None = True
            return [],[],[],[],[],[],[],[],is_None
